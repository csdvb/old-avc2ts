...
int Webcam::ConvertColor(unsigned char *out, unsigned char *in)
{
    ...
    unsigned char   *PlanV = PlanU + (yuv420frame.width * yuv420frame.height) /
                             4;
...
// printf("Image =%d/%d\n",buf.bytesused,buffers[buf.index].size);
    ...
    {
        ConvertColor(yuv420frame.data,
                     (unsigned char *) buffers[buf.index].data);
    }
    else
    {
...
void Webcam::open_device(void)
{
    ...
    {
        throw runtime_error(device + ": cannot identify! " + to_string(
    ...
    if (-1 == fd)
    {
        throw runtime_error(device + ": cannot open! " + to_string(
                                errno) + ": " + strerror(errno));
    }
}
...
void Webcam::init_device(void)
{
    ...
    if (force_format)
    {
        ...
        if (fmt.fmt.pix.pixelformat != V4L2_PIX_FMT_YUYV)
            ...
            throw runtime_error(
                      "Webcam does not support YUYV format. Support for more format need to be added!");
