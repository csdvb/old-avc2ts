...
namespace {
    ...
    static const char *format2str(OMX_COLOR_FORMATTYPE c)
    {
        ...
        case OMX_COLOR_FormatYUV411PackedPlanar:
            return
                "OMX_COLOR_FormatYUV411PackedPlanar: Planes fragmented when a frame is split in multiple buffers";
        case OMX_COLOR_FormatYUV420Planar:
            return "OMX_COLOR_FormatYUV420Planar: Planar YUV, 4:2:0 (I420)";
        case OMX_COLOR_FormatYUV420PackedPlanar:
            return
                "OMX_COLOR_FormatYUV420PackedPlanar: Planar YUV, 4:2:0 (I420), planes fragmented when a frame is split in multiple buffers";
        case OMX_COLOR_FormatYUV420SemiPlanar:
            return
                "OMX_COLOR_FormatYUV420SemiPlanar, Planar YUV, 4:2:0 (NV12), U and V planes interleaved with first U value";
        case OMX_COLOR_FormatYUV422Planar:
            return "OMX_COLOR_FormatYUV422Planar";
        case OMX_COLOR_FormatYUV422PackedPlanar:
            return
                "OMX_COLOR_FormatYUV422PackedPlanar: Planes fragmented when a frame is split in multiple buffers";
        ...
        case OMX_COLOR_FormatYUV420PackedSemiPlanar:
            return
                "OMX_COLOR_FormatYUV420PackedSemiPlanar: Planar YUV, 4:2:0 (NV12), planes fragmented when a frame is split in multiple buffers, U and V planes interleaved with first U value";
        case OMX_COLOR_FormatYUV422PackedSemiPlanar:
            return
                "OMX_COLOR_FormatYUV422PackedSemiPlanar: Planes fragmented when a frame is split in multiple buffers";
    ...
    {
        fprintf(stderr,
                "Port %d is %s, %s, buffers wants:%d needs:%d, size:%d, pop:%d, aligned:%d\n",
                portdef->nPortIndex,
                ...
                portdef->nBufferSize, portdef->bPopulated,
        ...
        case OMX_PortDomainImage:
            fprintf(stderr, "Image type:\n"
                    ...
                    format2str((OMX_VIDEO_CODINGTYPE) imgdef->
                               eCompressionFormat),
    ...
    static void printEvent(const char *compName, OMX_HANDLETYPE hComponent,
                           ...
                           OMX_U32 nData2)
    {
        ...
        if (strEvent)
            fprintf(stderr, "%s (%p) %s, data: %d, %d\n", compName, hComponent,
                    strEvent, nData1, nData2);
        else
            fprintf(stderr, "%s (%p) 0x%08x, data: %d, %d\n", compName,
                    hComponent, eEvent, nData1, nData2);
    }
}
...
private:
    ...
    {
public:
        VcosLock(VcosSemaphore *sem) : sem_(sem)
        {
            sem_->wait();
        }
        ~VcosLock()
...
namespace rpi_omx {
    ...
    static OMX_ERRORTYPE callback_EventHandler(OMX_HANDLETYPE hComponent,
                                               OMX_PTR pAppData,
    ...
    static OMX_ERRORTYPE callback_EmptyBufferDone(OMX_HANDLETYPE hComponent,
                                                  OMX_PTR pAppData,
                                                  OMX_BUFFERHEADERTYPE *
                                                  pBuffer);

    static OMX_ERRORTYPE callback_FillBufferDone(OMX_HANDLETYPE hComponent,
                                                 OMX_PTR pAppData,
                                                 OMX_BUFFERHEADERTYPE *
                                                 pBuffer);
...
public:
        ...
        OMXExeption(OMX_ERRORTYPE errCode, const char *file, unsigned line,
                    const char *msg = nullptr) : errCode_
                (errCode)
        {
            if (msg && msg[0])
                snprintf(msg_, MAX_LEN, "%s:%d OpenMAX IL error: 0x%08x. %s",
                         file, line, errCode, msg);
            else
                snprintf(msg_, MAX_LEN, "%s:%d OpenMAX IL error: 0x%08x", file,
                         line, errCode);
        }
        OMX_ERRORTYPE code() const
        {
            return errCode_;
        }
...
#define ERR_OMX(err, msg) if ((err) != OMX_ErrorNone) throw OMXExeption(err, \
                                                                        __FILE__, __LINE__, \
                                                                        msg)
...
// 7     640x480         4:3     40-90fps        x               Partial 2x2
    ...
    { 1920, 1080, 25, VideoFromat::RATIO_16x9, false };
    // static const VideoFromat VF_2560x1920 = { 2560, 1920, 0, VideoFromat::RATIO_4x3, true };
    static const VideoFromat    VF_1280x960 =
    { 1280, 960, 25, VideoFromat::RATIO_4x3, true };
    static const VideoFromat    VF_1280x720 =
    { 1280, 720, 25, VideoFromat::RATIO_16x9, true };
    static const VideoFromat    VF_640x480 =
    { 640, 480, 25, VideoFromat::RATIO_4x3, true };

    static const VideoFromat    VF_RESIZED_352x288 =
    { 352, 288, 25, VideoFromat::RATIO_4x3, true };
    static const VideoFromat    VF_RESIZED_640x480 = VF_640x480;
    static const VideoFromat    VF_RESIZED_320x240 =
    { 320, 240, 15, VideoFromat::RATIO_4x3, true };
    static const VideoFromat    VF_RESIZED_256x192 =
    { 256, 192, 25, VideoFromat::RATIO_4x3, true };
    static const VideoFromat    VF_RESIZED_160x120 =
    { 160, 120, 25, VideoFromat::RATIO_4x3, true };
    static const VideoFromat    VF_RESIZED_128x96 =
    { 128, 96, 25, VideoFromat::RATIO_4x3, true };

    static const VideoFromat    VF_RESIZED_960x540 =
    { 960, 540, 25, VideoFromat::RATIO_16x9, false };
    static const VideoFromat    VF_RESIZED_640x360 =
    { 640, 360, 25, VideoFromat::RATIO_16x9, false };
    static const VideoFromat    VF_RESIZED_480x270 =
    { 480, 270, 25, VideoFromat::RATIO_16x9, false };
    static const VideoFromat    VF_RESIZED_384x216 =
    { 384, 216, 25, VideoFromat::RATIO_16x9, false };
    static const VideoFromat    VF_RESIZED_320x180 =
    { 320, 180, 25, VideoFromat::RATIO_16x9, false };
    static const VideoFromat    VF_RESIZED_240x135 =
    { 240, 135, 25, VideoFromat::RATIO_16x9, false };
...
public:
    ...
    {
public:
        Buffer() : ppBuffer_(nullptr), fillDone_(false)
        {
        }
        bool filled() const
        {
        ...
        OMX_U8         *data()
        {
            return ppBuffer_->pBuffer + ppBuffer_->nOffset;
        }
        OMX_U32 dataSize() const
        {
            return ppBuffer_->nFilledLen;
        }
        OMX_U32 allocSize() const
        {
            return ppBuffer_->nAllocLen;
        }
        OMX_U32 TimeStamp()
        {
            return ppBuffer_->nTickCount;
        }
...
public:
        ...
        {
            return componentType2name(type_);
        }
        unsigned numPorts() const
        {
            return componentPortsCount(type_);
        }
        void dumpPort(OMX_U32 nPortIndex, OMX_BOOL dumpFormats =
                          OMX_FALSE)
        {
            ...
            if (dumpFormats)
            {
                ...
                std::
                cerr << "Port " << nPortIndex <<
                    " supports these video formats:" << std::endl;

                for (;; portformat->nIndex++)
                {
                    OMX_ERRORTYPE    err =
                        OMX_GetParameter(component_,
                    ...
                    std::cerr << "\t" << format2str(portformat->eColorFormat)
                              << ", compression: " << format2str(portformat->
                                                       eCompressionFormat)
                              << std::endl;
                }
            }
        ...
        void switchState(OMX_STATETYPE newState)
        {
            ...
            ERR_OMX(OMX_SendCommand
                        (component_, OMX_CommandStateSet, newState, NULL),
                    "switch state");

            if (!waitValue(&eventState_, value + 1))
                std::cerr << name() << " lost state changed event" << std::
                    endl;
#if 0
            if (!waitStateChanged(newState))
                std::
                cerr << name() << " state wanted: " << newState <<
                    " observed: " << state() << std::endl;
#endif
        }

        unsigned waitCount(OMX_U32 nPortIndex) const
        {
        ...
        void enablePort(OMX_U32 nPortIndex = OMX_ALL)
        {
            ...
            ERR_OMX(OMX_SendCommand
                        (component_, OMX_CommandPortEnable, nPortIndex, NULL),
                    "enable port");

            if (!waitValue(&eventEnabled_, value + waitCount(nPortIndex)))
                std::
                cerr << name() << " port " << nPortIndex <<
                    " lost enable port event(s)" << std::endl;
        }
        ...
        void disablePort(OMX_U32 nPortIndex = OMX_ALL)
        {
            ...
            ERR_OMX(OMX_SendCommand
                        (component_, OMX_CommandPortDisable, nPortIndex, NULL),
                    "disable port");

            if (!waitValue(&eventDisabled_, value + waitCount(nPortIndex)))
                std::
                cerr << name() << " port " << nPortIndex <<
                    " lost disable port event(s)" << std::endl;
        }
        ...
        void flushPort(OMX_U32 nPortIndex = OMX_ALL)
        {
            ...
            ERR_OMX(OMX_SendCommand
                        (component_, OMX_CommandFlush, nPortIndex, NULL),
                    "flush buffers");

            if (!waitValue(&eventFlushed_, value + waitCount(nPortIndex)))
                std::
                cerr << name() << " port " << nPortIndex <<
                    " lost flush port event(s)" << std::endl;
        }

        void getPortDefinition(OMX_U32 nPortIndex,
                               Parameter<
                                   OMX_PARAM_PORTDEFINITIONTYPE>
                               &portDef)
        {
            portDef->nPortIndex = nPortIndex;
            ERR_OMX(OMX_GetParameter
                        (component_, OMX_IndexParamPortDefinition, &portDef),
                    "get port definition");
        }

        void setPortDefinition(OMX_U32 nPortIndex,
                               Parameter<
                                   OMX_PARAM_PORTDEFINITIONTYPE>
                               &portDef)
        {
            portDef->nPortIndex = nPortIndex;
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamPortDefinition, &portDef),
                    "set port definition");
        }
        ...
        void allocBuffers(OMX_U32 nPortIndex, Buffer &buffer)
        {
            ...
            ERR_OMX(OMX_AllocateBuffer
                        (component_, buffer.pHeader(), nPortIndex, NULL,
                        portDef->nBufferSize), "OMX_AllocateBuffer");
        }

        void freeBuffers(OMX_U32 nPortIndex, Buffer &buffer)
        {
            ERR_OMX(OMX_FreeBuffer(component_, nPortIndex, buffer.header()),
                    "OMX_FreeBuffer");
        }

        void callFillThisBuffer(Buffer &buffer)
        {
            ERR_OMX(OMX_FillThisBuffer(component_, buffer.header()),
                    "OMX_FillThisBuffer");
        }

        void callEmptyThisBuffer(Buffer &buffer)
        {
            ERR_OMX(OMX_EmptyThisBuffer(component_, buffer.header()),
                    "OMX_EmptyThisBuffer");
        }
...
protected:
        ...
        Component(ComponentType type, OMX_PTR pAppData,
                  OMX_CALLBACKTYPE *callbacks) : type_(type),
            eventState_(0),
            eventFlushed_(0), eventDisabled_(0), eventEnabled_(0)
        {
            changedPorts_.resize(numPorts());

            OMX_STRING    xName = const_cast<OMX_STRING> (name());

            ERR_OMX(OMX_GetHandle(&component_, xName, pAppData, callbacks),
                    "OMX_GetHandle");
...
public:
        ...
        // The recommended initialisation sequence:// 1. Create component.// 2. Use OMX_IndexConfigRequestCallback to request callbacks on OMX_IndexParamCameraDeviceNumber.// 3. Set OMX_IndexParamISPTunerName.// 4. Set OMX_IndexParamCameraFlashType.// 5. Set OMX_IndexParamCameraDeviceNumber.// 6. Wait for the callback that OMX_IndexParamCameraDeviceNumber has changed.//      At this point, all the drivers have been loaded. Other settings can be applied whilst waiting for this event.// 7. Query for OMX_IndexConfigCameraSensorModes as required.// 8. Change state to IDLE, and proceed as required.
        Camera()
        ...
        void requestCallback()
        {
            ...
            ERR_OMX(OMX_SetConfig
                        (component_, OMX_IndexConfigRequestCallback, &cbtype),
                    "request callbacks");
        }
        ...
        void setDeviceNumber(unsigned camNumber)
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamCameraDeviceNumber, &device),
        ...
        void setVideoFromat(const VideoFromat &videoFormat)
        {
            ...
            portDef->format.video.nStride =
                align(portDef->format.video.nFrameWidth,
                      portDef->nBufferAlignment);
        ...
        void setFramerate(unsigned fps)
        {
            ...
            ERR_OMX(OMX_SetConfig
                        (component_, OMX_IndexConfigVideoFramerate, &framerate),
                    "set framerate");
        }

        // -100 .. 100
        void setSharpness(OMX_U32 nPortIndex =
                              OMX_ALL, OMX_S32 nSharpness = 0)
        {
            ...
            ERR_OMX(OMX_SetConfig
                        (component_, OMX_IndexConfigCommonSharpness,
                        &sharpness),
                    "set camera sharpness");
        }

        // -100 .. 100
        void setContrast(OMX_U32 nPortIndex =
                             OMX_ALL, OMX_S32 nContrast = 0)
        {
            ...
            ERR_OMX(OMX_SetConfig
                        (component_, OMX_IndexConfigCommonContrast, &contrast),
                    "set camera contrast");
        }

        // -100 .. 100
        void setSaturation(OMX_U32 nPortIndex =
                               OMX_ALL, OMX_S32 nSaturation = 0)
        {
            ...
            ERR_OMX(OMX_SetConfig
                        (component_, OMX_IndexConfigCommonSaturation,
                        &saturation),
                    "set camera saturation");
        }

        // 0 .. 100
        void setBrightness(OMX_U32 nPortIndex =
                               OMX_ALL, OMX_U32 nBrightness = 50)
        {
            ...
            ERR_OMX(OMX_SetConfig
                        (component_, OMX_IndexConfigCommonBrightness,
                        &brightness),
        ...
        void setExposureValue(OMX_U32 nPortIndex = OMX_ALL,
                              OMX_S32 xEVCompensation =
                                  0, OMX_U32 nSensitivity =
                                  100, OMX_BOOL bAutoSensitivity =
                                  OMX_TRUE)
        {
            ...
            ERR_OMX(OMX_SetConfig
                        ...
                        &exposure_value), "set camera exposure value");
        }

        void setFrameStabilisation(OMX_U32 nPortIndex =
                                       OMX_ALL, OMX_BOOL bStab =
                                       OMX_TRUE)
        {
            ...
            ERR_OMX(OMX_SetConfig
                        (component_, OMX_IndexConfigCommonFrameStabilisation,
                        &frame_stabilisation_control),
                    "set camera frame stabilisation");
        }

        void setWhiteBalanceControl(OMX_U32 nPortIndex =
                                        OMX_ALL,
                                        ...
                                        OMX_WhiteBalControlAuto)
        {
            ...
            ERR_OMX(OMX_SetConfig
                        (component_, OMX_IndexConfigCommonWhiteBalance,
                        &white_balance_control),
                    "set camera frame white balance");
        }

        void setImageFilter(OMX_U32 nPortIndex =
                                OMX_ALL,
                                ...
                                OMX_ImageFilterNone)
        {
            ...
            ERR_OMX(OMX_SetConfig
                        (component_, OMX_IndexConfigCommonImageFilter,
                        &image_filter), "set camera image filter");
        }

        // OMX_MirrorHorizontal | OMX_MirrorVertical | OMX_MirrorBoth
        void setMirror(OMX_U32 nPortIndex, OMX_MIRRORTYPE eMirror =
                           OMX_MirrorNone)
        {
            ...
            ERR_OMX(OMX_SetConfig
                        (component_, OMX_IndexConfigCommonMirror, &mirror),
                    "set cammera mirror");
        }
        ...
        void capture(OMX_U32 nPortIndex, OMX_BOOL bEnabled)
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexConfigPortCapturing, &capture),
...
public:
        ...
        Encoder() : Component(cType, (OMX_PTR) this, &cbsEvents)
        {
        }
        void setupOutputPortFromCamera(
            const Parameter<OMX_PARAM_PORTDEFINITIONTYPE> &cameraPortDef,
            unsigned bitrate,
            unsigned framerate = 0) // Framerate 0 means get it from Camera when tunelled
        {
            Parameter<OMX_PARAM_PORTDEFINITIONTYPE>    portDef;
            getPortDefinition(OPORT, portDef);

            portDef->format.video.nFrameWidth =
                cameraPortDef->format.video.nFrameWidth;
            portDef->format.video.nFrameHeight =
                cameraPortDef->format.video.nFrameHeight;
            portDef->format.video.xFramerate =
                cameraPortDef->format.video.xFramerate;
            portDef->format.video.nStride =
                cameraPortDef->format.video.nStride;
        ...
        void setupOutputPort(const VideoFromat Videoformat,
                             unsigned bitrate, unsigned framerate =
                                 25)
        {
            // Input Definition
            ...
            portDefI->format.video.eColorFormat =
                OMX_COLOR_FormatYUV420PackedPlanar;
        ...
        void setCodec(OMX_VIDEO_CODINGTYPE codec)
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamVideoPortFormat, &format),
                    "set video format");
        }
                             ...
                             int Level = OMX_VIDEO_AVCLevel4)
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamVideoProfileLevelCurrent,
                        &ProfileLevel), "set Profile/Level");
        }


        void setBitrate(OMX_U32 bitrate,
                        OMX_VIDEO_CONTROLRATETYPE type =
                            OMX_Video_ControlRateVariable)
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamVideoBitrate, &brate),
                    "set bitrate");
        }
        ...
        void setIDR(OMX_U32 idr_period /*, OMX_U32  nPFrames */)
        {
            ...
            ERR_OMX(OMX_GetParameter
                        (component_, OMX_IndexConfigVideoAVCIntraPeriod,
                        &idr_st),
                    " Get idr");
        ...
        void setVectorMotion()
        {
            ...
            ERR_OMX(OMX_SetParameter
                        ...
                        &motionvector), "set vector motion");
        }
        ...
        void setEED()
        {
            ...
            EED->enable = OMX_TRUE;
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamBrcmEEDEEnable, &EED),
                    "EED");
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamBrcmEEDELossRate, &EEDRate),
                    "EED");
        }
        ...
        void setSEIMessage()
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamBrcmVideoAVCSPSTimingEnable,
                        &SPSTiming), "SPS Timing Enable");

            Parameter<OMX_CONFIG_PORTBOOLEANTYPE>    SEIMessage;
            SEIMessage->nPortIndex = OPORT;
            SEIMessage->bEnabled = OMX_TRUE;
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamBrcmVideoAVCSEIEnable,
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamBrcmVideoAVC_VCLHRDEnable,
                        &VCLHRD), "VLCHRD");
        }
        ...
        void setSeparateNAL()
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamBrcmNALSSeparate, &SepNAL),
                    "Separate NAL");
        }
        ...
        void setMinizeFragmentation()
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexConfigMinimiseFragmentation,
                        &MinizeFragmentation), "Minimize Fragmentation");
        }
        ...
        void setMaxFrameLimits(int FrameLimitInBits)
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamBrcmVideoFrameLimitBits,
        ...
        void setPeakRate(int PeakRateinKbit)
        {
            ...
            ERR_OMX(OMX_GetParameter
                        (component_, OMX_IndexParamBrcmVideoPeakRate,
                        &PeakRate),
                    " GetPeakRate");
            printf("\nPEAK GET = %d\n", PeakRate->nU32);
        ...
        void setDynamicBitrate(int VideoBitrate)
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexConfigVideoBitrate,
                        &DynamicBitrate),
                    "DynamicVideoBitRate");
        }
...
// Set QP restrict QP : means if encoder choose a QP which is not in this range, Frame is dropped
        void setQPLimits(int QMin = 10, int QMax = 50)
        {
            ...
            QPMin->nU32 = QMin;
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamBrcmVideoEncodeMinQuant,
                        &QPMin), " QPMin");
            ...
            QPMax->nU32 = QMax;
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamBrcmVideoEncodeMaxQuant,
                        &QPMax), " QPMax");
        }
...
// ONLY IF RATECONTROL is not CBR/VBR
        void setQP(int QPi, int QPp)
        {
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexParamBrcmVideoEncodeQpP, &QP),
                    " QP");
        }
...
/* https://e2e.ti.com/support/embedded/android/f/509/t/234521 */
            ...
            ERR_OMX(OMX_GetParameter
                        (component_, OMX_IndexParamVideoIntraRefresh,
...
// SHOULD HAVE INSPECT WITH OMX_VIDEO_INTRAREFRESHTYPE
        ...
        void getEncoderStat(int Flags)
        {
            ...
            ERR_OMX(OMX_GetParameter
                        (component_, OMX_IndexConfigBrcmPortStats, &VideoStat),
                    " Get VideoStat");
            struct timespec    t;
            ...
            printf
            (
                "VideoStat : %s ByteCount %d Buffer %d - Frame %d = %d Skip %d Discard %d Max Delta%d:%d TIME %li\n",
                /*VideoStat->nByteCount.nLowPart*8*25/VideoStat->nFrameCount, */
                debug, VideoStat->nByteCount.nLowPart,
                VideoStat->nBufferCount, VideoStat->nFrameCount,
                VideoStat->nBufferCount - VideoStat->nFrameCount * 2,
                VideoStat->nFrameSkips, VideoStat->nDiscards,
                VideoStat->nMaxTimeDelta.nHighPart,
                VideoStat->nMaxTimeDelta.nLowPart,
                (t.tv_sec - tbefore.tv_sec) * 1000ul + (t.tv_nsec -
                                                        tbefore.tv_nsec) /
                1000000);
            tbefore = t;
            Count++;
...
public:
        ...
        NullSink() : Component(cType, (OMX_PTR) this, &cbsEvents)
        {
        }
    };
...
public:
        ...
        VideoRenderer() : Component(cType, (OMX_PTR) this, &cbsEvents)
        {
        }
        void setupPortFromCamera(const Parameter<
                                     OMX_PARAM_PORTDEFINITIONTYPE>
                                 &cameraPortDef)
        {
            Parameter<OMX_PARAM_PORTDEFINITIONTYPE>    portDef;
...
#define DISPLAY_DEVICE 0
            ...
            display_region->set =
                (OMX_DISPLAYSETTYPE) (OMX_DISPLAY_SET_NUM |
            ...
            ERR_OMX(OMX_SetParameter
                        (component_, OMX_IndexConfigDisplayRegion,
                        &display_region), " Display rect");
        }
...
public:
        ...
        VideoSplitter() : Component(cType, (OMX_PTR) this, &cbsEvents)
        {
        }
    };
...
public:
        ...
        Resizer() : Component(cType, (OMX_PTR) this, &cbsEvents)
        {
        }
        void setupOutputPort(int SrcImageWidth, int SrcImageHeight,
                             const VideoFromat Videoformat,
                             OMX_COLOR_FORMATTYPE colortype)
        {
// http://www.jvcref.com/files/PI/documentation/ilcomponents/resize.html
            ...
            if (colortype == OMX_COLOR_FormatYUV420PackedPlanar)
            {
                ...
                printf("%d * %d\n", portDefI->format.image.nStride,
                       portDefI->format.image.nSliceHeight);
            }
            else
            {
...
// Output definition
            ...
            portDefO->format.image.eColorFormat =
                OMX_COLOR_FormatYUV420PackedPlanar; // For encoder
...
public:
        ...
        ImageEncode() : Component(cType, (OMX_PTR) this, &cbsEvents)
        {
        }
        void setupPort(int SrcImageWidth, int SrcImageHeight,
                       OMX_COLOR_FORMATTYPE colortype)
        {
// http://www.jvcref.com/files/PI/documentation/ilcomponents/resize.html
            Parameter<OMX_PARAM_PORTDEFINITIONTYPE>    portDefI;
...
private:
                                               ...
                                               OMX_PTR pEventData)
    {
        ...
        case OMX_EventError:
            OMXExeption::die((OMX_ERRORTYPE) nData1,
                             "OMX_EventError received");
            break;
    ...
    static OMX_ERRORTYPE callback_EmptyBufferDone(OMX_HANDLETYPE,
                                                  OMX_PTR pAppData,
                                                  OMX_BUFFERHEADERTYPE *
                                                  pBuffer)
    {
        ...
        if (component->type() == ImageEncode::cType)
        {
            ImageEncode   *imageencode =
                static_cast<ImageEncode *>(pAppData);
...
public:
    ...
    void ConstructTsTree(int VideoBit, int TsBitrate, int PMTPid,
                         char *sdt, int fps = 25)
    {
        VideoPid = PMTPid + 1;
        Videofps = fps;
        ...
        ts_setup_sdt(writer);
        ts_setup_mpegvideo_stream(writer, VideoPid, 32, // 3.2
                                  AVC_BASELINE,         // Fixme should pass Profile and Level
                                  VideoBitrate, 40000,  // Fix Me : should have to be calculated
                                  Videofps);
        if (OutputFilename)
                  ...
                  int DelayPTS = 200, struct timespec *Time = NULL)
    {
        ...
        // if(OmxFlags&OMX_BUFFERFLAG_ENDOFNAL)
        {
            ...
            if (OmxFlags & OMX_BUFFERFLAG_SYNCFRAME)
                tsframe.frame_type =
                    LIBMPEGTS_CODING_TYPE_SLICE_IDR |
            ...
            TimeToTransmitFrameUs =
                (TotalFrameSize * 8.0 * 1000000.0 * 1.1 /
                 (float)MaxVideoBitrate);
            // if(OmxFlags&OMX_BUFFERFLAG_SYNCFRAME)
            if (Time == NULL)                             // Frame base calculation
            {
                ...
                tsframe.cpb_initial_arrival_time =
                    vdts * 300L - TimeToTransmitFrameUs * 2.7 -
                    DelayPTS * 90 * 300L;
                tsframe.cpb_final_arrival_time =
                    vdts * 300L - TimeToTransmitFrameUs * 2.7 -
                    DelayPTS * 90 * 300L;
            }
            else
            ...
            if (key_frame > 1)       // Skip first frame
                ret =
                    ts_write_frames(writer, &tsframe, 1, &out, &len,
                                    &pcr_list);
            else
                len = 0;
            if (len)
            ...
            {
                fprintf(stderr,
                        "tswrite frame Len=0 Ret=%d tsframe.size=%d originalsize=%d\n",
...
#define BUFF_MAX_SIZE (7 * 188)
        ...
        while (len > 0)
        {
            if (Size + len >= BUFF_MAX_SIZE)
            {
                ...
                if (sendto
                        (m_sock, Buffer, BUFF_MAX_SIZE, 0,
                        (struct sockaddr *)&m_client, sizeof(m_client)) < 0)
                {
...
public:
              ...
              int EnableMotionVectors = 0)
    {
        ...
        // configuring encoders
        {
            ...
            tsencoder.ConstructTsTree(VideoBitrate, TsBitrate, PMTPid, sdt,
                                      fps);
        ...
        ERR_OMX(OMX_SetupTunnel
                    (camera.component(), Camera::OPORT_VIDEO,
                    encoder.component(),
                    Encoder::IPORT), "tunnel camera.video -> encoder.input");
        ERR_OMX(OMX_SetupTunnel
                    (camera.component(), Camera::OPORT_PREVIEW,
                    videorender.component(), VideoRenderer::IPORT),
    ...
    void Run(bool want_quit)
    {
        ...
        if (!want_quit && encBuffer.filled())
        {
            ...
            if (encBuffer.flags() & OMX_BUFFERFLAG_CODECSIDEINFO)
            {
                ...
                for (j = 0; j < CurrentVideoFormat.height / 16; j++)
                {
                    for (i = 0; i < CurrentVideoFormat.width / 16; i++)
                    {
                        int Motionx =
                            encBuffer.
                            data()[(CurrentVideoFormat.width / 16 * j +
                                    i) * 4];
                        int Motiony =
                            encBuffer.
                            data()[(CurrentVideoFormat.width / 16 * j +
            ...
            if (toWrite)
            {
                ...
                clock_gettime(CLOCK_REALTIME, &gettime_now);
                gettime_now.tv_sec =
                    (int)difftime(gettime_now.tv_sec, InitTime.tv_sec);
...
public:
              ...
              int ModeInput = Mode_PATTERN, char *Extra = NULL)
    {
        ...
        if (Mode == Mode_GRABDISPLAY)
        {
            ...
            resizer.setupOutputPort(DisplayWidth, DisplayHeight, VideoFormat,
...
//                                CurrentVideoFormat.height);
        ...
        ERR_OMX(OMX_SetupTunnel
                    (resizer.component(), Resizer::OPORT, encoder.component(),
                    Encoder::IPORT),
        ...
        if (Mode == Mode_GRABDISPLAY)
        {
            pgrabdisplay->SetOmxBuffer((unsigned char *)resizer.inBuffer().
                                       data());
        }
        if (Mode == Mode_VNCCLIENT)
        {
            pvncclient->SetOmxBuffer((unsigned char *)resizer.inBuffer().
                                     data());
        }
...
#define MARGIN 500
        ...
        long BigToSleepns =
            (MuToSleep * 1000L - time_difference - KERNEL_GRANULARITY);
        // printf("ToSleep %ld\n",BigToSleepns/1000);
...
// generate an animated test card in YUV format
                           ...
                           int frame)
    {
        ...
        frame = frame % 256;
        OMX_U8 *y = buf, *u =
            y + CurrentVideoFormat.width * CurrentVideoFormat.height, *v =
            u +
            (CurrentVideoFormat.width >> 1) * (CurrentVideoFormat.height >> 1);
        ...
        *filledLen =
            ((CurrentVideoFormat.width * CurrentVideoFormat.height * 3) / 2);

        return 1;
    }

    int generate_test_rgbcard(OMX_U8 *buf, OMX_U32 *filledLen,
                              int frame)
    {
        OMX_U8 *current = buf;
        unsigned int i, j;
...
//                      generate_test_card(PictureBuffer.data(),&filledLen,key_frame);
            if (Mode == Mode_PATTERN)
            {
                generate_test_card(PictureBuffer.data(), &filledLen,
                                   key_frame);
            ...
            if (Mode == Mode_GRABDISPLAY)
            {
                ...
                pgrabdisplay->GetDisplaySize(DisplayWidth, DisplayHeight,
                                             Rotate);
...
// printf("Avnt %ld:%ld - %ld:%ld \n",gettime_now.tv_sec,gettime_now.tv_nsec,InitTime.tv_sec,InitTime.tv_nsec);
                ...
                {
                    gettime_now.tv_nsec =
                        ((int64_t) 1E9L + (int64_t) gettime_now.tv_nsec) -
                        (int64_t) InitTime.tv_nsec;
                    gettime_now.tv_sec -= 1;
                }
                else
                {
                    gettime_now.tv_nsec =
                        gettime_now.tv_nsec - (int64_t) InitTime.tv_nsec;
                }
 ...
 */
    ...
    try
    {
        ...
        if (TypeInput == 0)
        {
            ...
            cameratots->Init(CurrentVideoFormat, OutputFileName, NetworkOutput,
                             VideoBitrate, MuxBitrate, DelayPTS, pidpmt, sdt,
                             VideoFramerate, IDRPeriod, RowBySlice, false);
        }
        else
        {
            ...
            picturetots->Init(CurrentVideoFormat, OutputFileName,
                              NetworkOutput, VideoBitrate, MuxBitrate,
                              DelayPTS, pidpmt, sdt, VideoFramerate, IDRPeriod,
                              RowBySlice, false, PictureMode, ExtraArg);
        }
#if 1
        ...
        std::
        cerr << "Enter capture and encode loop, press Ctrl-C to quit..." <<
            std::endl;
        ...
        while (1)
        {
            ...
            if (want_quit
                /*&& (encBufferLow.flags() & OMX_BUFFERFLAG_SYNCFRAME) */)
            {
        ...
        std::cerr << "high: " << highCount << " low: " << lowCount << std::
            endl;
